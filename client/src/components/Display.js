                           ...
                           const images = await Promise.all(
                                ...
                                const images1= await Promise.all(str1.map(async(item,i)=>{
                                    return(
                                        ...
                                        key={i} 
                                        src={`https://indigo-subjective-spider-731.mypinata.cloud/ipfs${item.substring(6)}`}
                                        alt="new"
                                        className="image-list"/>)}
                                        <div className='test11'>
                                
                                        <img src={downloadIcon} onClick={()=>downl(`https://indigo-subjective-spider-731.mypinata.cloud/ipfs${item.substring(6)}` , str2[i])}/>    
                                        </div> 
                                        <div className='test12'>
                                        <a href={`https://indigo-subjective-spider-731.mypinata.cloud/ipfs${item.substring(6)}`} key={i} target="_blank">{str2[i]}</a>
                                        </div> 
                                        </div>
                                        </a>
                                    )
